...
make build    # Production build: Hugo minify (search index generated by Hugo natively)
```
...
- Keyboard shortcuts (⌘+E/J/I/Y/B/K, `/` to open panel; ⌘K opens search overlay)
...
### Search
Craig Mod's Hugo Fast Search — no build step required, works in both `make serve` and `make build`.
- Index: `layouts/_default/index.json` → served as `/index.json` (Hugo JSON output format, configured in `config.yaml`)
- JS: `static/js/fastsearch.js` (plain script, not bundled through Hugo pipeline)
- Overlay HTML: injected in `layouts/partials/foot.html`
- Triggered by ⌘K or clicking the Search link in the nav panel
- Index covers: `blog`, `notes`, `links`, `page`, and root-level pages (excludes `books`, `courses`, etc.)
