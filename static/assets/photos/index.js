...
if (process.argv.length !== 4) {
    console.error("usage: massage image.jpg image")
    process.exit(0);
}
...
(async function() {
    for (let res of [128, 640, 1280, 2880]) {
        const jpegOutoutFilename = `${prefix}_${res}.jpg`;
        const webpOutputFilename = `${prefix}_${res}.webp`;

        await sharp(inputBuffer)
            .resize(res)
            .toFile(jpegOutoutFilename);
        ...
        console.log(
            `${res}x jpg ${prettyBytes(
                fs.readFileSync(jpegOutoutFilename).byteLength
            )}`
        );
    }
})();